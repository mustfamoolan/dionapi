...
كل عميل يمر بثلاث حالات رئيسية:
...
- **انتهاء التفعيل:** إذا انتهت مدة التفعيل (`activation_expires_at` أصبح في الماضي)، سيعود العميل إلى حالة غير نشطة. يجب على الإدارة إعادة تفعيل الحساب.
...
3. إذا كانت الحالة `active` ولكن `activation_expires_at` قد انتهى → يتم رفض تسجيل الدخول برسالة: "حسابك غير مفعل أو انتهت مدة التفعيل. يرجى التواصل مع الدعم."
4. إذا كانت الحالة `active` ولم تنتهِ مدة التفعيل → يتم تسجيل الدخول بنجاح.
